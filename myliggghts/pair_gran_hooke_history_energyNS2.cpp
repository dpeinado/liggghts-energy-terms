...
PairGranHookeHistoryEnergyNS2::PairGranHookeHistoryEnergyNS2(LAMMPS *lmp) : PairGranHookeHistoryEnergy(lmp)
{
...
void PairGranHookeHistoryEnergyNS2::compute(int eflag, int vflag, int addflag)
{
        ...
        if (shearupdate)
         ...
         fs3 = - (kt*shear[2]+gammat*vtr3);
          ...
          else
          {
              ...
              myWorkT= -((fs1+dfv1)*dTx+(fs2+dfv2)*dTy+(fs3+dfv3)*dTz);
