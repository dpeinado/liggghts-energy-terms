...
{
  //delete history fix
  if (MESHGRAN_FWG) modify->delete_fix("fix_neigh_tri");
  //unregister energy terms as property/peratom
  if (fpgIKE!=NULL) modify->delete_fix("IKE");
}
...
void FixWallGran::post_create()
{
    ...
    //register Temp as property/peratom
      ...
      fpgIKE=static_cast<FixPropertyGlobal*>(modify->fix[modify->find_fix_property("IKE","property/global","scalar",0,0)]);
    }
    delete []fixarg;
}
      ...
      if (wallstyle != MESHGRAN_FWG){
          ...
          if (rsq > radius[i]*radius[i])
          {
        	  if(touch[i]){
        		 touch[i]=0;
        		 if(history) {
        			 c_history = contacthistory[i][0];
        			 reset_contact(i,c_history);
        		 }
        	  }
...
void FixWallGran::grow_arrays(int nmax)
{
  ...
  touch     = (int*)(memory->srealloc(touch    , nmax*sizeof(int), "fix_wall_gran:touch"));
...
void FixWallGran::unpack_restart(int nlocal, int nth)
{
  ...
  touch[nlocal] = static_cast<int>(extra[nlocal][m++]);
