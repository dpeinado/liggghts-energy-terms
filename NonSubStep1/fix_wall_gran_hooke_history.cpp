...
void FixWallGranHookeHistory::compute_force(int ip, double deltan, double rsq,double meff_wall, double dx, double dy, double dz,double *vwall,double *c_history, double area_ratio)
{
  ...
  fs1 = - (kt*c_history[0]+ gammat*vtr1);
  fs2 = - (kt*c_history[1]+ gammat*vtr2);
  fs3 = - (kt*c_history[2]+ gammat*vtr3);
  ...
  if (fs > fn) {
      if (shrmag != 0.0) {
          ...
          fs2 *= fn/fs;
          fs3 *= fn/fs;
          c_history[0]=-(fs1+gammat*vtr1)/kt;
          c_history[1]=-(fs2+gammat*vtr2)/kt;
          c_history[2]=-(fs3+gammat*vtr3)/kt;
      }
